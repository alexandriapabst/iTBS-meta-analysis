...
```
No significant differences between subgroups - although we can qualitatively see that studies that utilize AMT vs RMT for cognitive enhancement generally have a bit more positive of an effect.

Now we will look at differences of stimulator type.

```{r}
stimtype.subgroup <- update.meta(m.dlbj, byvar = meta.data$StimType, comb.random = TRUE, comb.fixed = FALSE)
...
```{r}
coildiam.subgroup <- update.meta(m.dlbj, byvar = meta.data$CoilDiameter, comb.random = TRUE, comb.fixed = FALSE)
...
```{r}
nav.subgroup <- update.meta(m.dlbj, byvar = meta.data$NavigationtoLocation, comb.random = TRUE, comb.fixed = FALSE)
forest(nav.subgroup)
...
```{r}
subgroup.analysis.mixed.effects(x = m.dlbj, 
...
```{r}
subgroup.analysis.mixed.effects(x = m.dlbj, subgroups = meta.data$ShamCond)
```
...
```{r}
subgroup.analysis.mixed.effects(x = m.dlbj, subgroups = meta.data$Location)
```
...
```{r}
metareg(m.dlbj, ShamCond)
```
The R^2 for this model is 4.22%...this is not doing a good job at explaining the variance across effect sizes. We will do this for each individual variable, because I think it's important to see how much each variable explains.

Next, we will do a metaregression for $Construct.

```{r}
metareg(m.dlbj, Construct)
```
...
```{r}
output.metareg <- metareg(m.dlbj, Location)
...
output.metareg
```
Okay, this is where things really get interesting. We knew that Location was found to be a significant predictor using random effects modeling. When performing a meta regression, we found that Location explains 57.11% of the variance across effect sizes! That's a significant portion, and something that is really important to communicate and discuss in the conclusions.

Now, we will investigate study design.

```{r}
metareg(m.dlbj, StudyDesign)
```
This model doesn't actually seem to account much for explaining the variation in effect sizes (contrary to previous findings in the random effects modeling), as it's R^2 value is 6.56%.

Next, we will investigate the influence of Coil Diameter on effect sizes. 

```{r}
coildiam.metareg <- metareg(m.dlbj, CoilDiameter)
print(coildiam.metareg)
bubble(coildiam.metareg, xlab = "Coil Diameter",
       col.line = "blue", lwd = 2,
       studlab  = TRUE)
```
One of the continuous variables that we can investigate and use as a predictor in a meta-regression is the diameter of the coil. This model does not explain any of the variance in effect sizes.

Next, we will investigate the percentage of stimulator output and its relation to effect sizes.

```{r}
mt.metareg <- metareg(m.dlbj, MT.)
...
Next, we will investigate the influence of Mean Age on effect sizes.
```{r}
age.metareg <- metareg(m.dlbj, MeanAge)
bubble(age.metareg, xlab = "Mean Age of Sample",
       col.line = "blue", lwd = 2,
       studlab  = TRUE)
print(age.metareg)
```
This is interesting - the model here says that the R^2 value is 58.25%, and that mean age of the participants in the studies do have an influence on effect sizes. This needs to be investigated much more thoroughly, as I need to make sure that the model is treating mean age as a continuous predictor and not a categorical predictor.

In the output here, we see that the youngest age (which is 20.1) is the intercept.This warrants a graph that displays the entire model (*do this)
However, when looking at how the significant estimates compare to the intercept, it is clear that this is due to the small number of studies representing each category.

Next, I will look at using AMT versus RMT to stimulate and its influence on effect sizes.
```{r}
metareg(m.dlbj, AMTvsRMT)
```
...
```{r}
metareg(m.dlbj, StimType)

```
This model determines that stimulator type does not have any influence on the distribution of effect sizes.

Next, I will look at the role of current flow in effect sizes (AP versus PA).

```{r}
output.metareg <- metareg(m.dlbj, CurrentFlow)
bubble(output.metareg,
       xlab = "Current Flow",
       col.line = "blue",
       studlab = TRUE)
output.metareg
```
This model determiens that Current Flow determines approximately 6.49% of the variance in effect sizes. 
...
There are, however, certain things that we need to watch out for when building these powerful models. 
  ...
  2. Next, we need to watch out for multicolliniearity - which means that one or more predictor in the model can be linearly predicted from another predictor within the model with relatively high accuracy (highly correlated predictors). This will limit the R^2 value of the odel, and will also report false positives. We can deal with multicollinearity by removing redundant predictors or combining multiple predictors into one.
...
multimodel.inference(TE = "Adjustments",
                     seTE = "SE_effectsize",
                     data = meta.data,
                     predictors = c("Construct","ShamCond","Location"),
                     interaction = FALSE)
```
...
What we see in the plot above is that there is somewhat of a likelihood of publication bias toward larger studies, as the majority of studies are at the top with very little standard error. 

```{r}
funnel(m.dlbj, xlab = "Hedges' g", studlab = TRUE)
```
...
In the funnel plot above, we can see that many studies actually report non significance, but especially smaller studies. Studies with larger groups are the ones that end up reporting more significance, but this seems to be pretty well rounded for all studies, with what looks to be studies reporting non-significance at a higher rate than those that are significant.
